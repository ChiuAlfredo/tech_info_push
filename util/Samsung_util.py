...
def crawl_product(company, keyword):
    try:
        ...
        logging.error(f"Error in crawling {company} {keyword} : {e}")
...
def crawl_detail(company, keyword):
...
def crawl_compar(company, keyword):
    ...
    for item in compare_data_list:
        # item = compare_data_list[0]
        transformed_dict = {}
        transformed_dict['product_name'] = item['id'].replace('-',' ')
        for i in item['features'].keys():
            # i='speakers'
            try:
                transformed_dict[i.lower()] = item['features'][i]['text'].replace('<br/>', '\n')
            except Exception as e:
                logging.error(e)
...
def detail_laptop(company, keyword):
    ...
    def set_fpr(row):
        ...
        if "finger" in str(row["security_x"]).lower() or "finger" in str(row["security_y"]).lower():
            return "Yes"
